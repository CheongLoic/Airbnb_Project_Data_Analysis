...
server <- function(session,input, output) {
  ...
  observeEvent(
    ...
    switch(
      ...
      availability_30 = updateSliderInput(session, "slider", 
                                          min = 0,#min(mydata_filtered_by_date_tab1()$availability_30), 
                                          max = 30, #max(mydata_filtered_by_date_tab1()$availability_30),
                                          value=  c(0,30)#c(min(mydata_filtered_by_date_tab1()$availability_30),max(mydata_filtered_by_date_tab1()$availability_30))
                                          ), 
  ...
  observeEvent(
    ...
    # # Dynamically update the date choices based on the selected city
    # updateSelectizeInput(session, "select_date2", 
    #                      choices = mydata$date[mydata$city == input$select_city],
    #                      selected = NULL,  # Reset the selected date
    #                      server = TRUE)    # Use server-side rendering for better performance
  ...
  output$menu <- renderMenu({
    sidebarMenu(
      ...
      menuItem("Analysis 1 - Comparing cities", tabName="tab1", icon = icon("chart-bar")),
      menuItem("Analysis 2 - Deep dive into city", tabName="tab2", icon = icon("chart-bar"))
    )
  })
